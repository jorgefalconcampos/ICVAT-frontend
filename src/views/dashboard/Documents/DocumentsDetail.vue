              ...
              <v-dialog v-model="dialog" width="500">
                ...
                <v-card>
                  <v-card-title class="text-h5 red lighten-1"
                    >¿Borrar el documento&nbsp;<span class="font-weight-bold">{{
                      document_data.title
                    }}</span
                    >?</v-card-title
                  >
                  <v-card-text class="mt-4"
                    >Al continuar se eliminará de tus documentos guardados. Ten
                    en cuenta que esta acción no se puede deshacer.</v-card-text
                  >
                  <v-divider></v-divider>
                  <v-card-actions class="py-3">
                    ...
                    >
                    <v-btn
                      @click="deleteDocument(document_data.document_id)"
                      color="px-3 red lighten-1 white--text"
                      dense
                      rounded
                      >Sí, eliminar</v-btn
                    >
          ...
          <div v-else class="bgblue border15 mx-5 py-5">
            ...
            >
              <v-col cols="3" class="mx-2">
                <h1 class="glass border10 text-overline">
                  Creado: {{ document_data.created_date }}
                </h1>
              </v-col>
              <v-col cols="3" class="mx-2">
                <h1 class="glass border10 text-overline">
                  Modificado: {{ document_data.created_date }}
                </h1>
              </v-col>
            </v-row>
            ...
            <div class="border15 mx-2 mx-sm-10 pa-5 px-2 px-sm-15">
              <div v-if="editMode">
                ...
                <v-form ref="form" v-model="isValid" @submit.prevent="submit">
                  ...
                  <vue-editor
                    class="grey lighten-5 pa-3 text-left border10 a-default"
                    v-model="form.body"
                  ></vue-editor>
                    ...
                    <template v-slot:no-data class="mt-3">
                      <v-list-item class="mt-3">
                        <v-list-item-content class="mt-3">
                          <v-list-item-title>
                            No hay tags que coincidan: "<strong>{{
                              search
                            }}</strong
              ...
              <div v-else>
                    ...
                    <h3
                      @click="
                        $router.push({
                          ...
                          params: { data: { id: document_data.category } },
                        })
                      "
                      class="text-h4 py-1 px-10 clickable mx-12 glass-white-border"
                    >
                ...
                <v-divider></v-divider>
                <div
                  class="grey lighten-5 my-5 pa-3 text-left border10 a-default"
                  v-dompurify-html="document_data.body"
                ></div>
                <v-chip
                  v-for="(tag, i) in document_data.tags"
                  :key="`tag_${i}`"
  ...
  methods: {
    submit() {
      if (this.$refs.form.validate()) {
        ...
        this.isNew
          ? this.createDoc()
          : this.updateDoc(this.document_data.document_id);
      } else {
        this.showSnackbar(
    ...
    async createDoc() {
      try {
        ...
        const response = await client.documents
          .createDocument(myHeaders, this.form)
          .then((r) =>
    ...
    async updateDoc(uuid) {
      try {
        ...
        const myHeaders = new Headers({ Authorization: `Bearer ${token}` });
        const response = await client.documents
          .updateDocument(myHeaders, uuid, this.form)
          .then((r) =>
            r.text().then((data) => ({ status: r.status, body: data }))
          );
    ...
    async getDocument() {
      try {
        ...
        const myHeaders = new Headers({ Authorization: `Bearer ${token}` });
        const response = await client.documents
          .getSingleDocument(myHeaders, uuid)
          .then((r) =>
            r.text().then((data) => ({ status: r.status, body: data }))
          );
        if (response.status == 200) {
          ...
          this.document_categories_select.name =
            this.document_data.category_name;
    ...
    async getCategories() {
      try {
        ...
        this.showSnackbar(
          ["Ocurrió un error al obtener información del documento"],
          "red",
          true,
          true,
          "mdi-alert-circle",
      ...
      try {
        ...
        if (response.status == 200) {
          ...
          data.forEach((element) => {
            this.document_tags_all.push(element["name"]);
          });
        }
      } catch (err) {
        this.showSnackbar(
          ["Ocurrió un error al obtener información documento"],
          "red",
          true,
          true,
          "mdi-alert-circle",
